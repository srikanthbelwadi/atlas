...
def classify_and_plan(question: str, candidates: list[dict], pack: str = packs.DEFAULT_PACK) -> tuple[dict, dict]:
    ...
    prompt = (
        ...
        "Shape ad-hoc SQL for the answer step, which can only be handed a "
        "few hundred rows: aggregate (GROUP BY, SUM/AVG/COUNT) when the "
        "question asks for a summary, trend or ranking rather than listing "
        "raw records; ORDER BY the dimension the question cares about — "
        "most recent first for time, largest first for rankings — and end "
        "with LIMIT 500 or fewer, because the ordering decides which rows "
        "survive if the result is large. Confirmed live: an unordered "
        "multi-entity result was cut positionally and the last entity's "
        "most recent rows were lost.\n\n"
        + packs.glossary(pack)
...
def synthesize(question: str, evidence: dict, pack: str = packs.DEFAULT_PACK) -> tuple[dict, dict]:
    ...
    prompt = (
        ...
        "If the evidence carries a `coverage` object with truncated=true, "
        "you are seeing rows_sent of rows_total rows (an equal share per "
        "`stratified_by`, most recent first): say so in one sentence, "
        "describe what you were given, and never claim the answer is "
