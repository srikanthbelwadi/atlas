<!-- Mirrored from the repo root IMPLEMENTATION.md for the /implementation page — keep both in sync when editing. -->
...
**2 hand-authored OKF docs** (`okf-catalog/`), the only entries in the catalog
...
- `ac.covid19_case_rate_by_county_year` — a `human-reviewed` Attested
  ...
  year," the top trust tier and the only templated path in the system today.
  Narrow by design — one question shape, one table.
...
**Inherited but inactive: Resource Raiser's original ~20 non-BigQuery
sources** (SEC, Census, Treasury, IRS Form 990, CDC, federal grants). The
...
none are queryable today. Restating the gap already called out below: the
"~20 authoritative sources" Resource Raiser offered are currently zero for
Atlas — every real answer Atlas gives today comes from the 14 BigQuery
dataset groups and 2 OKF docs above.
