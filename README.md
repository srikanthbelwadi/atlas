...
- [x] Google Data Commons in the public catalog: two reviewed templates (`ac.dc_indicator_for_place`, `ac.dc_indicator_across_places`) via `backend/accessor/datacommons_accessor.py` — places and variables resolved by Data Commons' own resolvers, one source facet per answer, receipts carry the resolved DCIDs; 16 unit tests; golden sets `places_a` 9/9, `public_regression` 10/10 and `public_b` 11/13 live; asked from the same ask bar as everything else — the home page's 26 example chips are grouped by data domain and every one is a passed golden question; discovery caps crawled candidates at two per dataset so template sources are never crowded out by table vintages
- [x] Map answers: `choropleth` visualization with boundaries from Data Commons place nodes, MapLibre GL, linked ranked table; the model chooses the kind and names columns, the backend attaches geometry (`backend/geo/`); golden set `maps_a` 7/7
- [x] Google Earth Engine through BigQuery: `ST_REGIONSTATS` templates for ERA5-Land climate and Hansen forest cover by county on the existing guarded executor, answers say they are computed estimates; golden set `ee_a` 6/6; 28 example chips in eight sections
...
API sources are added the way SEC EDGAR was: one OKF document plus one accessor module (`backend/accessor/sec_edgar_accessor.py` is the template); `_fetch_one()` in `pipeline.py` is the dispatch point. Data Commons (`backend/accessor/datacommons_accessor.py`, two templates in the public catalog) is the second worked example: needs `DC_API_KEY` in the environment (Secret Manager on Cloud Run; see `scripts/datacommons_phase0.sh`). The `on_user_created` Cloud Function referenced in `main.py`'s docstring is built — see `infra/functions/on_user_created/`.
