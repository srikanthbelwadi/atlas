...
- [x] Places pack (`/places`): Google Data Commons behind two reviewed templates (`ac.dc_indicator_for_place`, `ac.dc_indicator_across_places`) via `backend/accessor/datacommons_accessor.py` — places and variables resolved by Data Commons' own resolvers, one source facet per answer, receipts carry the resolved DCIDs; 16 unit tests; golden set `tests/golden/places_a.yaml` (run live via `scripts/places_phase0.sh`, then promote and flip `NEXT_PUBLIC_ATLAS_PLACES_ENABLED`)
...
API sources are added the way SEC EDGAR was: one OKF document plus one accessor module (`backend/accessor/sec_edgar_accessor.py` is the template); `_fetch_one()` in `pipeline.py` is the dispatch point. Data Commons (`backend/accessor/datacommons_accessor.py`, the `places` pack) is the second worked example: needs `DC_API_KEY` in the environment (Secret Manager on Cloud Run; see `scripts/places_phase0.sh`). The `on_user_created` Cloud Function referenced in `main.py`'s docstring is built — see `infra/functions/on_user_created/`.
